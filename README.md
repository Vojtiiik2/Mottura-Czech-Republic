...
Produkty Venkovní systémy, Designové systémy a Látky používají nově dodané fotografie. Zbývající produktové fotografie lze později nahradit změnou hodnoty `img` u příslušného produktu v souboru `app.js`. Náhledové obrázky katalogů jsou uložené samostatně ve složce `assets/img/catalogs/`.
