...
function openProductModal(index) {
  ...
  document.querySelector("#modalUses").innerHTML = item.uses
    .map((use) => `<li>${use}</li>`)
    .join("");
  document.querySelector("#modalNote").textContent = item.catalogUrl
    ? "Tento detail slouží jako české vysvětlení produktové skupiny. Přesné technické parametry a dostupné varianty doporučujeme ověřit v přiloženém katalogu nebo na oficiálním webu výrobce."
    : "Tento detail slouží jako základní české vysvětlení produktové skupiny. Konkrétní kolekce, vlastnosti a dostupnost látek se upřesňují podle požadavků daného projektu.";
